                    ...
                    let location = data["location"]!
                     ...
                     Database.database().reference(fromURL: realTimeDataBaseReference).child("conversations").childByAutoId().childByAutoId().setValue(withValues, withCompletionBlock: { (error, reference) in
                        let data = ["location": reference.parent!.key]
                         Database.database().reference(fromURL: realTimeDataBaseReference).child("users").child(currentUserID).child("conversations").child(toID).updateChildValues(data)
                         Database.database().reference(fromURL: realTimeDataBaseReference).child("users").child(toID).child("conversations").child(currentUserID).updateChildValues(data)
                        completion(true)
                    })
                }
            })
